...
import {
  ...
  UNIT5_TIMED_CHECKPOINT_QUESTION_IDS,
...
test('shared AP Chemistry registry validates all three Unit 5 draft blueprints', () => {
  ...
  assert.ok(unit5AssessmentBlueprints.every(({ review }) => review.status === 'draft' && review.reviewers.length === 0))
})

test('Unit 5 URLs resolve through the shared assessment registry and reject unknown IDs', () => {
  assert.equal(
    createApChemistryAssessmentUrl(UNIT5_DIAGNOSTIC_ID),
    `/questions.html?topic=ap-chemistry&domain=${DOMAIN_ID}&assessment=${UNIT5_DIAGNOSTIC_ID}`,
  )
  assert.equal(getApChemistryAssessmentBlueprint(UNIT5_DIAGNOSTIC_ID)?.domainId, DOMAIN_ID)
  assert.equal(createApChemistryAssessmentUrl('invented-unit-5-assessment'), null)
})

test('Unit 5 diagnostic and reassessment are disjoint parallel forms over six skills', () => {
  assert.equal(UNIT5_DIAGNOSTIC_QUESTION_IDS.length, 6)
  assert.equal(UNIT5_REASSESSMENT_QUESTION_IDS.length, 6)
  assert.equal(UNIT5_DIAGNOSTIC_QUESTION_IDS.filter((id) => UNIT5_REASSESSMENT_QUESTION_IDS.includes(id)).length, 0)
  const skills = (ids) => ids.map((id) => questionById.get(id).taxonomy.skillId).sort()
  assert.deepEqual(skills(UNIT5_DIAGNOSTIC_QUESTION_IDS), skills(UNIT5_REASSESSMENT_QUESTION_IDS))
})

test('Unit 5 checkpoint is locked, timed, and spans all eleven topics with two manual FRQs', () => {
  const blueprint = unit5AssessmentBlueprints.find(({ kind }) => kind === 'unit-test')
  const selected = UNIT5_TIMED_CHECKPOINT_QUESTION_IDS.map((id) => questionById.get(id))
...
test('generic Unit 5 result summary recommends canonical resources and diagnoses misconceptions', () => {
  const blueprint = getApChemistryAssessmentBlueprint(UNIT5_DIAGNOSTIC_ID)
  const questions = UNIT5_DIAGNOSTIC_QUESTION_IDS.map((id) => questionById.get(id))
  const grade = {
  ...
  assert.equal(summary.id, UNIT5_DIAGNOSTIC_ID)
  assert.equal(summary.recommendations.length, 1)
  assert.equal(summary.recommendations[0].skillId, 'reaction-rates')
  ...
  assert.ok(summary.misconceptionIds.includes('kinetics-negative-rate-value'))
  assert.match(summary.errorPatterns[0].correction, /nonnegative magnitude/)
